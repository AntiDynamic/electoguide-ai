...
Write-Host "`n🗳️  ElectoGuide AI — Cloud Run Deployment" -ForegroundColor Cyan
...
Write-Host "2️⃣  Enabling required Cloud APIs..." -ForegroundColor Yellow
...
Write-Host "3️⃣  Building Docker image with Cloud Build..." -ForegroundColor Yellow
...
gcloud run deploy $SERVICE_NAME `
    ...
    --min-instances 0 `
    --max-instances 10 `
    --set-env-vars "GEMINI_API_KEY=$GEMINI_KEY,GEMINI_MODEL=gemini-1.5-flash,GCP_PROJECT_ID=$PROJECT_ID,ENVIRONMENT=production" `
    --project $PROJECT_ID

# ── Step 5: Get service URL ───────────────────────────────────────────────────
Write-Host "`n5️⃣  Fetching service URL..." -ForegroundColor Yellow
